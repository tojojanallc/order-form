...
export default function CreateEventPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      alert(`DB ERROR ${error.code}: ${error.message}. Change the slug slightly.`);
      setLoading(false);
    } else {
  ...
  return (
    <div className="min-h-screen bg-white text-black p-8 max-w-2xl mx-auto border-x-4 border-black">
      <Link href="/admin" className="text-blue-600 font-black uppercase text-[10px] hover:underline italic">← Command Center</Link>
      ...
      <form onSubmit={handleCreate} className="space-y-8 bg-gray-50 p-8 border-4 border-black shadow-[12px_12px_0px_0px_black]">
        ...
        <div className="grid grid-cols-2 gap-6">
          <button 
            type="button" 
            onClick={() => setForm(prev => ({...prev, mode: 'retail'}))} 
          ...
          <button 
            type="button" 
            onClick={() => setForm(prev => ({...prev, mode: 'hosted'}))} 
