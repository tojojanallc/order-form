...
export default function EventReportPage({ params }: { params: { slug: string } }) {
  ...
  async function fetchDetails(slug: string) {
    ...
    try {
      ...
      if (eventOrders) {
        ...
        // We sum the 'finalPrice' of every item in every cart to get the real value,
        // even if the order total was recorded as $0 for a Hosted Event.
        const calculatedValue = eventOrders.reduce((totalSum, order) => {
            const cart = Array.isArray(order.cart_data) ? order.cart_data : [];
            const orderValue = cart.reduce((itemSum: number, item: any) => {
                return itemSum + (Number(item.finalPrice) || 0);
            }, 0);
            return totalSum + orderValue;
        }, 0);

        setRevenue(calculatedValue);
      }
    } catch (err: any) {
    ...
    filteredOrders.forEach(order => {
      ...
      cart.forEach((item: any) => {
        reportData.push({
          ...
          'Unit Value': item.finalPrice || 0, // Individual item value
          'Total Order Value': order.total_price > 0 ? order.total_price : item.finalPrice // Fallback for hosted
        });
      });
    });
  ...
  if (loading) return <div className="min-h-screen bg-gray-50 flex items-center justify-center font-black text-gray-400 animate-pulse uppercase tracking-widest">Calculating Value...</div>;
              ...
              <div className={`p-8 rounded-[40px] shadow-2xl text-right min-w-[320px] ${isHosted ? 'bg-indigo-900' : 'bg-slate-950'}`}>
                ...
                {isHosted && (
                    <p className="text-[9px] font-bold text-indigo-400 uppercase mt-2 tracking-widest italic">
                        * Calculated from Item Prices + Customizations
                    </p>
                )}
              </div>
          </div>
             ...
             <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                    {filteredOrders.length === 0 ? (
                        <tr><td colSpan={3} className="p-32 text-center font-black text-gray-300 uppercase italic tracking-widest">No matches found</td></tr>
                    ) : filteredOrders.map(order => {
                        // For Hosted Mode, we calculate the order's value for the table row
                        const cart = Array.isArray(order.cart_data) ? order.cart_data : [];
                        const rowValue = cart.reduce((sum: number, item: any) => sum + (Number(item.finalPrice) || 0), 0);
                        ...
                        return (
                            <tr key={order.id} className="hover:bg-blue-50/10 transition-colors">
                                        ...
                                        {cart.map((item: any, i: number) => (
                                            <div key={i} className="border-l-4 border-blue-500 pl-4 py-1">
                                                <div className="text-xs font-black text-slate-800 uppercase flex items-center gap-2">
                                                    ...
                                                    <span className="text-gray-400 font-bold ml-auto">${item.finalPrice}</span>
                                                </div>
                                ...
                                <td className="p-8 text-right font-black text-slate-900 text-xl tracking-tighter">
                                    ${rowValue.toFixed(2)}
                                </td>
                            </tr>
                        );
