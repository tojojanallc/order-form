...
export default function Reconcile() {
  ...
  const returnAllToWarehouse = async () => {
    ...
    if (itemsToProcess.length === 0) return alert("No items with stock > 0 found to return.");
    ...
    for (const item of itemsToProcess) {
      try {
        // 1. Fetch current Warehouse stock for this SKU
        ...
        if (fetchErr || !master) {
          console.error(`SKU ${item.product_id} not found in master inventory.`);
          continue; 
        }

        // 2. Add Event count to Warehouse count
        const newWarehouseTotal = (master.quantity_on_hand || 0) + item.count;
        const { error: updateMasterErr } = await supabase
          .from('inventory_master')
          .update({ quantity_on_hand: newWarehouseTotal })
          .eq('sku', item.product_id);

        if (updateMasterErr) throw updateMasterErr;

        // 3. Zero out the Event Kiosk count
        ...
        console.error(`Failed to reconcile ${item.product_id}:`, err.message);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-white text-black p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-8 p-6 bg-gray-100 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <label className="block text-sm font-black uppercase mb-2">Event to Close</label>
        <select 
            ...
            {eventStock.map(s => (
              <tr key={s.id} className={s.count === 0 ? "opacity-30" : ""}>
                <td className="p-4 font-black uppercase">
                  ...
                  <div className="text-[10px] text-gray-500">{s.product_id}</div>
                </td>
