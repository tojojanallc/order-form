...
export default function ReturnToVendor() {
  ...
  const addToReturn = () => {
    ...
    if (amount > item.quantity_on_hand) return alert(`Not enough stock. Only ${item.quantity_on_hand} available.`);
    ...
    try {
      ...
      for (const item of cart) {
        ...
        await supabase
          ...
          .update({ quantity_on_hand: currentItem.quantity_on_hand - item.qty })
          .eq('sku', item.sku);
      }
            ...
            <div className="col-span-8 space-y-6">
                ...
                <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm min-h-[400px]">
                    ...
                    <div className="grid grid-cols-12 gap-4 items-end mb-6 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                        ...
                        <div className="col-span-2">
                            <button onClick={addToReturn} className="w-full h-[48px] bg-slate-900 text-white font-black uppercase text-[10px] tracking-widest rounded-xl hover:bg-red-500 transition-colors">Add</button>
                        </div>
                    </div>
                ...
                <div className="bg-slate-900 text-white p-6 rounded-[30px] shadow-2xl sticky top-6">
                    ...
                    <div className="border-t border-slate-800 pt-6 mt-4">
                        <button 
                            ...
                            className={`w-full py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all ${loading || cart.length === 0 ? 'bg-slate-800 text-slate-600 cursor-not-allowed' : 'bg-red-600 text-white hover:bg-red-500 shadow-lg shadow-red-500/20'}`}
                        >
