...
export default function OrderHelperPage() {
  ...
  const generate = async () => {
    setLoading(true);
    setResults(null);
    const { data: orders } = await supabase.from('orders').select('id, cart_data, total_price, payment_status, status, event_slug').neq('status', 'refunded');
    if (!orders) { setLoading(false); return; }
    const paidOrders = orders.filter(o => { const p = (o.payment_status || '').toLowerCase(); return p === 'paid' || p === 'succeeded' || Number(o.total_price) === 0; });
    ...
    const avgItemsPerEvent = Object.values(itemsPerEvent).length > 0 ? Object.values(itemsPerEvent).reduce((s, n) => s + n, 0) / Object.values(itemsPerEvent).length : 1;
    ...
    if (parsedSchedule?.divisions?.length > 0) {
      scheduleRecs = parsedSchedule.divisions.map((div: any) => {
        ...
        sizeInfo.sizes.forEach((size, i, arr) => {
          const isYouthSize = SIZE_ORDER.indexOf(size) < 4;
          const pool = isYouthSize ? youthAthletes : adultAthletes;
          const weight = i === Math.floor(arr.length / 2) ? 0.4 : 0.3;
          sizeBreakdown[size] = Math.max(1, Math.ceil(pool * weight * (1 + bufferPct)));
        });
    ...
    const recommendations = Object.entries(salesMap)
      ...
      .map(([productName, sizeCounts]) => {
        ...
        const sizes = SIZE_ORDER.filter(s => sizeCounts[s]).map(s => {
          ...
          const scaled = attendees > 0 ? Math.ceil((historicalQty / uniqueEvents) * scaleFactor * (1 + bufferPct)) : Math.ceil((historicalQty / uniqueEvents) * (1 + bufferPct));
          return { size: s, historicalQty, pct, recommended: Math.max(1, scaled) };
        });
        return { productName, totalSold, avgPerEvent: totalSold / uniqueEvents, sizes, totalRecommended: sizes.reduce((s, r) => s + r.recommended, 0) };
      });

    setResults({ recommendations, avgItemsPerEvent, scheduleRecs, estimatedAthletes: attendees });
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-gray-50 font-sans p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-4">
          ...
          <p className="text-xs text-gray-400 mb-4">Upload a PDF with age divisions and team counts for age-specific size recommendations.</p>
          <div className="flex gap-3 items-center flex-wrap">
            <input ref={fileRef} type="file" accept=".pdf" className="hidden" onChange={e => { setPdfFile(e.target.files?.[0] || null); setParsedSchedule(null); }} />
            <button onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-gray-300 hover:border-blue-400 rounded-xl px-6 py-3 text-sm font-bold text-gray-500 hover:text-blue-600 transition-all">
              {pdfFile ? `📄 ${pdfFile.name}` : '+ Upload PDF'}
            </button>
            {pdfFile && !parsedSchedule && <button onClick={parsePdf} disabled={pdfParsing} className="bg-blue-600 hover:bg-blue-700 text-white font-black px-6 py-3 rounded-xl text-sm disabled:opacity-40 transition-all">{pdfParsing ? '⏳ Reading PDF...' : '🔍 Parse Schedule'}</button>}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 font-bold focus:outline-none focus:border-blue-400" placeholder="15" value={buffer} onChange={e => setBuffer(e.target.value)} />
              <p className="text-xs text-gray-400 mt-1">Extra % added to all quantities</p>
            </div>
            <div className="flex items-end">
              <button onClick={generate} disabled={loading} className="w-full bg-slate-900 hover:bg-slate-700 text-white font-black py-3 rounded-xl disabled:opacity-40 transition-all">
          ...
          <>
            ...
            {results.scheduleRecs?.length > 0 && (
              <div className="mb-6">
                ...
                <p className="text-xs text-gray-400 mb-4">Size ranges estimated from age group norms. Use as a guide for distributing blanks across youth vs adult.</p>
            ...
            <p className="text-xs text-gray-400 mb-4">Based on your actual sales across all events, scaled to expected attendance.</p>
