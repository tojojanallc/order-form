...
export default function MasterAdmin() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      // 3. Today's Revenue (Estimate from orders for simplicity, or your ledger)
    ...
    <div className="min-h-screen bg-gray-50 p-8 font-sans">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Link href="/admin/inventory/warehouse" className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-md transition-all border border-gray-100 flex flex-col justify-between h-full group">
          <div>
            <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
              <span className="text-2xl">🏬</span>
            </div>
            <h2 className="text-xl font-bold text-gray-900 mb-2">Warehouse Master</h2>
            <p className="text-gray-500 text-sm leading-relaxed">Central product catalog. Update images, SKUs, and verify total physical stock.</p>
          </div>
        ...
        <Link href="/admin/inventory/reconcile" className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-md transition-all border border-gray-100 flex flex-col justify-between h-full group">
          <div>
            ...
            <p className="text-gray-500 text-sm leading-relaxed">Return unsold event stock back to the master warehouse and reconcile counts.</p>
          </div>
        ...
        <Link href="/admin/inventory/adjust" className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-md transition-all border border-gray-100 flex flex-col justify-between h-full group">
          <div>
            ...
            <p className="text-gray-500 text-sm leading-relaxed">Manually adjust master counts for write-offs, damages, or shrinkage.</p>
          </div>
      ...
      {/* Bottom Section */}
      <div className="max-w-7xl mx-auto mt-12 border-t border-gray-200 pt-10">
        <div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
           <div>
              <h3 className="text-xl font-black text-gray-900">Event Configuration</h3>
              <p className="text-gray-500">Manage onsite kiosks, branding, colors, and payment gateway settings.</p>
           </div>
           <Link href="/admin/events" className="bg-gray-900 text-white px-10 py-4 rounded-2xl font-black hover:bg-slate-800 transition-all shadow-lg hover:shadow-xl active:scale-95">
