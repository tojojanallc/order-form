...
export default function AdminPage() {
  ...
  // --- 1. REAL-TIME LISTENER (FORCING REFRESH) ---
  useEffect(() => {
        ...
        if (supabase) {
            const channel = supabase
              ...
              .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
                  console.log("DB Change detected, refreshing list...");
                  fetchOrders(); // This forces the list to update on the screen
              })
              .subscribe();
  ...
  useEffect(() => {
    // Bookmark the starting count so it doesn't print all history on refresh
    ...
    if (orders.length > lastOrderCount.current) {
      ...
      // Only print if the order was created in the last 60 seconds
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === 'orders' && ( <div className="space-y-6"> 
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4"> 
            ...
            {/* AUTO-PRINT UI INSERTED INTO ORDER TAB STATS */}
        ...
        {activeTab === 'inventory' && (
            <div className="grid md:grid-cols-3 gap-6">
                <div className="md:col-span-1 space-y-6">
                    <div className="bg-white p-6 rounded-lg shadow border border-gray-200"><h2 className="font-bold text-xl mb-4 text-black">Add New Item</h2><form onSubmit={handleAddProductWithSizeUpdates} className="space-y-3"><div><label className="text-xs font-bold uppercase text-gray-500">ID</label><input className="w-full border p-2 rounded text-black" placeholder="jogger_navy" value={newProdId} onChange={e => setNewProdId(e.target.value)} /></div><div><label className="text-xs font-bold uppercase text-gray-500">Display Name</label><input className="w-full border p-2 rounded text-black" placeholder="Navy Joggers" value={newProdName} onChange={e => setNewProdName(e.target.value)} /></div><div><label className="text-xs font-bold uppercase text-gray-500">Type</label><select className="w-full border p-2 rounded bg-white text-black" value={newProdType} onChange={e => setNewProdType(e.target.value)}><option value="top">Top</option><option value="bottom">Bottom</option></select></div><button className="w-full bg-green-600 text-white font-bold py-2 rounded">Create Product</button></form></div>
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        {editingOrder.cart_data.map((item, idx) => (
                            <div key={idx} className="bg-white p-4 border rounded-lg shadow-sm space-y-4 text-black">
                                ...
                                <div><label className="text-xs font-bold uppercase text-gray-400">Design</label><select className="w-full border p-2 rounded text-black font-bold" value={item.customizations?.mainDesign || ''} onChange={(e) => handleUpdateMainDesign(idx, e.target.value)} ><option value="">None</option>{logos.filter(l => l.category === 'main').map(l => ( <option key={l.id} value={l.label}>{l.label}</option> ))}</select></div>
