...
export default function AdminPage() {
  ...
  // --- 1. REAL-TIME LISTENER (FIXED REFRESH) ---
  useEffect(() => {
        ...
        if (supabase) {
            const channel = supabase
              ...
              .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
                  ...
                  fetchOrders(); // This forces the actual refresh
              })
              .subscribe();
  ...
  // --- 2. AUTO-PRINT TRIGGER (FIXED BASELINE) ---
  useEffect(() => {
    // Bookmark the starting count on load so we don't print history
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === 'orders' && ( <div className="space-y-6"> 
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300 overflow-x-auto"> 
            <table className="w-full text-left min-w-[800px]"><thead className="bg-gray-200"><tr><th className="p-4 uppercase text-xs text-gray-500 font-bold">Status</th><th className="p-4 uppercase text-xs text-gray-500 font-bold">Customer</th><th className="p-4 uppercase text-xs text-gray-500 font-bold">Items</th><th className="p-4 text-right font-bold uppercase text-xs text-gray-500 font-bold">Actions</th></tr></thead><tbody>{orders.filter(o => o.status !== 'completed').map((order) => {
        ...
        {activeTab === 'inventory' && ( <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-1 space-y-6">
                <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
                    <h2 className="font-bold text-xl mb-4 text-black uppercase tracking-tighter">Add Item</h2>
                    <form onSubmit={handleAddProductWithSizeUpdates} className="space-y-3">
                        ...
                        <input className="w-full border p-2 rounded text-black" placeholder="Image URL" value={newProdImage} onChange={e => setNewProdImage(e.target.value)} />
                ...
                <div className="bg-blue-50 p-6 rounded-lg shadow border border-blue-200">
                    ...
                    <button onClick={downloadTemplate} className="text-xs bg-white border border-blue-300 px-3 py-1 rounded text-blue-700 font-bold mb-4 uppercase">⬇️ Download CSV</button>
            ...
            <div className="md:col-span-2 space-y-6"><div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300"><div className="bg-blue-900 text-white p-4 font-bold uppercase text-xs tracking-widest">Pricing Console</div><table className="w-full text-left text-sm text-black"><thead className="bg-gray-100"><tr><th className="p-3 font-bold uppercase text-xs">Product</th><th className="p-3 font-bold uppercase text-xs text-center">Price ($)</th></tr></thead><tbody>{products.map((prod) => (<tr key={prod.id} className="border-b"><td className="p-3 font-bold">{prod.name}</td><td className="p-3 font-bold text-center">$<input type="number" className="w-20 border rounded p-1 ml-1 text-black font-bold shadow-sm text-center" value={prod.base_price} onChange={(e) => updatePrice(prod.id, e.target.value)} /></td></tr>))}</tbody></table></div><div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300"><div className="bg-gray-800 text-white p-4 font-bold uppercase text-xs tracking-widest">Stock & Costs</div><table className="w-full text-left text-sm text-black"><thead className="bg-gray-100"><tr><th className="p-4 uppercase text-xs font-bold text-gray-500">Product</th><th className="p-4 uppercase text-xs font-bold text-gray-500">Size</th><th className="p-4 uppercase text-xs font-bold text-center text-gray-500">Cost ($)</th><th className="p-4 uppercase text-xs font-bold text-center text-gray-500">Stock</th></tr></thead><tbody>{inventory.map((item) => (<tr key={`${item.product_id}_${item.size}`} className="border-b"><td className="p-4 font-bold">{getProductName(item.product_id)}</td><td className="p-4 font-medium">{item.size}</td><td className="p-4"><input type="number" className="w-16 border rounded text-center mx-auto block text-black shadow-sm" value={item.cost_price || ''} onChange={(e) => updateStock(item.product_id, item.size, 'cost_price', parseFloat(e.target.value))} /></td><td className="p-4"><input type="number" className="w-16 border text-center font-bold text-black mx-auto block shadow-sm" value={item.count} onChange={(e) => updateStock(item.product_id, item.size, 'count', parseInt(e.target.value))} /></td></tr>))}</tbody></table></div></div>
        </div> )}

        {activeTab === 'guests' && (<div className="max-w-4xl mx-auto"><div className="bg-white p-6 rounded-lg shadow mb-6 border border-gray-200"><h2 className="font-bold text-xl mb-4 text-black uppercase tracking-widest">Guests</h2><input type="file" onChange={handleGuestUpload} className="block w-full text-sm text-gray-500 mb-4" /><button onClick={clearGuestList} className="mt-4 text-red-600 font-bold text-xs uppercase underline tracking-tighter">🗑️ Clear List</button></div><div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300"><table className="w-full text-left text-sm text-black"><thead className="bg-gray-100"><tr><th className="p-4 uppercase text-xs text-gray-500 font-bold">Name</th><th className="p-4 uppercase text-xs text-gray-500 font-bold text-center">Pre-Size</th><th className="p-4 text-center uppercase text-xs text-gray-500 font-bold">Status</th><th className="p-4 text-right uppercase text-xs text-gray-500 font-bold">Action</th></tr></thead><tbody>{guests.map((g) => (<tr key={g.id} className="border-b"><td className="p-4 font-bold">{g.name}</td><td className="p-4 font-mono font-bold text-blue-600 text-center">{g.size || '-'}</td><td className="p-4 text-center">{g.has_ordered ? <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full font-black text-[10px]">REDEEMED</span> : 'Waiting'}</td><td className="p-4 text-right"><button onClick={() => resetGuest(g.id)} className="text-blue-600 font-bold text-xs underline uppercase">Reset</button></td></tr>))}</tbody></table></div></div>)}
        
        {activeTab === 'logos' && (<div className="max-w-4xl mx-auto"><div className="bg-white p-6 rounded-lg shadow mb-6 border border-gray-200"><h2 className="font-bold text-xl mb-4 text-black uppercase tracking-widest">Logo Repository</h2><form onSubmit={addLogo} className="grid grid-cols-2 gap-4"><input className="border p-2 rounded text-black col-span-2 shadow-sm" placeholder="Label Name" value={newLogoName} onChange={e => setNewLogoName(e.target.value)}/><input className="border p-2 rounded text-black col-span-2 shadow-sm" placeholder="Image URL" value={newLogoUrl} onChange={e => setNewLogoUrl(e.target.value)}/><div className="col-span-2 flex items-center gap-6 bg-gray-50 p-2 rounded border border-gray-200"><span className="font-bold text-gray-700 text-sm">Type:</span><label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="cat" checked={newLogoCategory === 'main'} onChange={() => setNewLogoCategory('main')} className="w-4 h-4" /><span className="text-sm font-bold text-black uppercase">Main Design</span></label><label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="cat" checked={newLogoCategory === 'accent'} onChange={() => setNewLogoCategory('accent')} className="w-4 h-4" /><span className="text-sm font-bold text-black uppercase">Accent</span></label></div><button className="bg-blue-900 text-white font-bold p-3 col-span-2 rounded uppercase tracking-widest shadow-lg">Add Logo</button></form><div className="mt-6 space-y-2 text-black">{logos.map(l => (<div key={l.id} className="flex justify-between p-2 border-b items-center"><span className="font-bold">{l.label} <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded ml-2 uppercase tracking-tighter">{l.category}</span></span><button onClick={() => deleteLogo(l.id)} className="text-red-500 font-bold text-lg">🗑️</button></div>))}</div></div>)}

        {activeTab === 'settings' && (<div className="max-w-xl mx-auto bg-white p-8 rounded shadow border border-gray-300 space-y-6"><h2 className="font-black text-2xl text-black uppercase tracking-tighter">Settings</h2><div><label className="font-bold text-xs uppercase text-gray-500">Event Title</label><input className="w-full border p-3 rounded text-black font-bold shadow-sm" value={eventName} onChange={e => setEventName(e.target.value)} /></div>
            <div className="mb-6 bg-gray-100 p-4 rounded border border-gray-200 text-black"><label className="block font-bold mb-3 border-b pb-2 uppercase text-xs">Output Format</label><div className="space-y-2"><label className="flex items-center gap-3 cursor-pointer"><input type="radio" checked={printerType === 'label'} onChange={() => setPrinterType('label')} className="w-5 h-5" /><span className="font-bold text-sm">Thermal Label (4x6)</span></label><label className="flex items-center gap-3 cursor-pointer"><input type="radio" checked={printerType === 'standard'} onChange={() => setPrinterType('standard')} className="w-5 h-5" /><span className="font-bold text-sm">Standard Sheet (8.5x11)</span></label></div></div>
            <div className="mb-6 bg-blue-50 p-4 rounded border border-blue-200 text-black"><label className="block font-bold mb-3 border-b pb-2 uppercase text-xs">Payment Mode</label><div className="space-y-2"><label className="flex items-center gap-3 cursor-pointer"><input type="radio" checked={paymentMode === 'retail'} onChange={() => setPaymentMode('retail'} className="w-5 h-5" /><span className="font-bold text-sm">Retail (Stripe)</span></label><label className="flex items-center gap-3 cursor-pointer"><input type="radio" checked={paymentMode === 'hosted'} onChange={() => setPaymentMode('hosted'} className="w-5 h-5" /><span className="font-bold text-sm">Hosted (Party Mode)</span></label></div></div>
            <div className="bg-purple-50 p-6 rounded border border-purple-200 text-black shadow-inner"><p className="font-bold text-purple-900 mb-2 uppercase text-[10px] tracking-widest border-b border-purple-200 pb-2">Cloud PrintNode</p><div className="flex justify-between items-center mb-4"><span>Enable?</span><input type="checkbox" checked={pnEnabled} onChange={e => setPnEnabled(e.target.checked)} className="w-6 h-6" /></div><input className="w-full border p-2 mb-2 text-sm text-black shadow-sm" placeholder="API Key" value={pnApiKey} onChange={e => setPnApiKey(e.target.value)}/><div className="flex gap-2"><input className="flex-1 border p-2 text-sm text-black shadow-sm" placeholder="Printer ID" value={pnPrinterId} onChange={e => setPnPrinterId(e.target.value)} /><button onClick={discoverPrinters} className="bg-purple-600 text-white px-3 py-1 text-xs rounded font-bold uppercase shadow">Scan</button></div></div><button onClick={saveSettings} className="w-full bg-blue-900 text-white py-4 font-black rounded shadow-xl uppercase tracking-widest hover:bg-blue-800 transition-all">Save Changes</button></div>)}

        {/* --- Archive --- */}
        {activeTab === 'history' && ( <div> <div className="bg-gray-800 text-white p-4 rounded-t-lg flex justify-between items-center shadow-lg"><h2 className="font-bold text-xl uppercase tracking-widest">Archive</h2><button onClick={downloadCSV} className="bg-white text-black px-4 py-2 rounded font-bold hover:bg-gray-200 text-xs uppercase tracking-tighter">📥 Export</button></div> <div className="bg-white shadow rounded-b-lg overflow-hidden border border-gray-300 overflow-x-auto"> {orders.filter(o => o.status === 'completed').length === 0 ? <div className="p-8 text-center text-gray-400 font-bold uppercase italic">History empty.</div> : ( <table className="w-full text-left min-w-[800px]"> <thead className="bg-gray-100 text-gray-500"> <tr> <th className="p-4 font-bold text-xs uppercase">Event</th> <th className="p-4 font-bold text-xs uppercase">Customer</th> <th className="p-4 text-right font-bold text-xs uppercase">Total</th> </tr> </thead> <tbody> {orders.filter(o => o.status === 'completed').map((order) => ( <tr key={order.id} className="border-b hover:bg-gray-50 opacity-75"> <td className="p-4 font-bold text-blue-900">{order.event_name || '-'}</td> <td className="p-4 font-bold text-gray-900">{order.customer_name}</td> <td className="p-4 text-right font-black text-green-900">${order.total_price}</td> </tr> ))} </tbody> </table>)} </div> </div> )}
