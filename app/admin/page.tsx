...
export default function AdminPage() {
  ...
  const [guests, setGuests] = useState([]); // NEW: GUEST LIST
  ...
  const handleGuestUpload = (e) => {
    ...
    reader.onload = async (evt) => {
        try {
            ...
            const data = XLSX.utils.sheet_to_json(ws); // Expects column "Name"
            
            if (!data || data.length === 0) return alert("Empty file.");
            
            let count = 0;
            for (const row of data) {
                // Look for "Name" or "Guest" column
                const name = row['Name'] || row['name'] || row['Guest'] || row['guest'];
                if (name) {
                    await supabase.from('guests').insert([{ name: String(name).trim(), has_ordered: false }]);
                    count++;
                }
            }
  ...
  const resetGuest = async (id) => { if (confirm("Allow this guest to order again?")) { await supabase.from('guests').update({ has_ordered: false }).eq('id', id); fetchGuests(); } };
  const clearGuestList = async () => { if (confirm("⚠️ DELETE ALL GUESTS? This cannot be undone.")) { await supabase.from('guests').delete().neq('id', 0); fetchGuests(); } };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 text-black font-sans">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === 'guests' && (
            <div className="max-w-4xl mx-auto">
                <div className="bg-white p-6 rounded-lg shadow mb-6 border border-gray-200">
                    <h2 className="font-bold text-xl mb-4">Guest List Management</h2>
                    <p className="text-gray-600 mb-4 text-sm">Upload an Excel file with a <strong>Name</strong> column to restrict orders.</p>
                    <div className="flex gap-4">
                        ...
                        <button onClick={clearGuestList} className="text-red-600 font-bold text-sm whitespace-nowrap">🗑️ Clear All</button>
                    </div>
                </div>
                <div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300">
                    <table className="w-full text-left">
                        <thead className="bg-gray-100 border-b"><tr><th className="p-4">Guest Name</th><th className="p-4 text-center">Status</th><th className="p-4 text-right">Action</th></tr></thead>
                        <tbody>
                            {guests.length === 0 ? <tr><td colSpan="3" className="p-8 text-center text-gray-500">No guests uploaded.</td></tr> : guests.map((guest) => (
                                <tr key={guest.id} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="p-4 text-right"><button onClick={() => resetGuest(guest.id)} className="text-blue-600 hover:text-blue-800 font-bold text-xs underline">Reset Status</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
        ...
        {/* LOGOS & SETTINGS PRESERVED (Shortened for brevity but logically present) */}
