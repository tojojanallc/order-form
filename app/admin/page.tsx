      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          ...
          <div className="flex bg-white rounded-lg p-1 shadow border border-gray-300">
            <button onClick={() => { setActiveTab('orders'); fetchOrders(); }} className={`px-6 py-2 rounded font-bold transition-colors ${activeTab === 'orders' ? 'bg-blue-900 text-white shadow' : 'text-gray-600 hover:bg-gray-100'}`}>Orders</button>
            <button onClick={() => { setActiveTab('inventory'); fetchInventory(); }} className={`px-6 py-2 rounded font-bold transition-colors ${activeTab === 'inventory' ? 'bg-blue-900 text-white shadow' : 'text-gray-600 hover:bg-gray-100'}`}>Inventory</button>
          </div>
        </div>

        {activeTab === 'orders' && (
            <div>
                <div className="flex justify-end mb-4 gap-2"><button onClick={downloadCSV} className="bg-green-600 text-white px-4 py-2 rounded font-bold hover:bg-green-700">📥 Export CSV</button><button onClick={fetchOrders} className="bg-gray-200 text-gray-800 px-4 py-2 rounded font-bold hover:bg-gray-300">Refresh</button></div>
                {loading ? <p>Loading...</p> : (
                <div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300 overflow-x-auto">
                    ...
                    <tbody>
                        ...
                        <tr key={order.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="p-4 align-top"><div className="font-bold">{order.customer_name}</div><div className="text-sm">{order.phone}</div>{order.shipping_address && <div className="mt-2 text-sm bg-purple-50 p-2 rounded border border-purple-200 text-purple-900">🚚 {order.shipping_address}</div>}</td>
                            <td className="p-4 align-top text-sm">{order.cart_data.map((item, i) => <div key={i} className="mb-2"><strong>{item.productName}</strong> ({item.size}) {item.needsShipping && "🚚"}</div>)}</td>
                        </tr>
                        ))}
                    </tbody>
                    </table>
        ...
        {activeTab === 'inventory' && (
            <div>
                <div className="flex justify-end mb-4"><button onClick={fetchInventory} className="bg-gray-200 text-gray-800 px-4 py-2 rounded font-bold hover:bg-gray-300">Refresh Stock</button></div>
                <div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300">
                    <table className="w-full text-left">
                    <thead className="bg-gray-800 text-white"><tr><th className="p-4">Product Name</th><th className="p-4">Size</th><th className="p-4 w-48">Stock Count</th><th className="p-4 text-center">Active?</th></tr></thead>
                    <tbody>
                        {inventory.map((item) => (
                        <tr key={`${item.product_id}_${item.size}`} className={`border-b ${!item.active ? 'bg-gray-100 opacity-60' : 'hover:bg-gray-50'}`}>
                            ...
                            <td className="p-4"><div className="flex items-center gap-2"><button onClick={() => updateStock(item.product_id, item.size, 'count', Math.max(0, item.count - 1))} className="w-8 h-8 bg-gray-200 rounded font-bold hover:bg-gray-300">-</button><input type="number" className="w-20 border-2 border-gray-300 text-center p-1 rounded font-bold text-lg" value={item.count} onChange={(e) => updateStock(item.product_id, item.size, 'count', parseInt(e.target.value) || 0)} /><button onClick={() => updateStock(item.product_id, item.size, 'count', item.count + 1)} className="w-8 h-8 bg-gray-200 rounded font-bold hover:bg-gray-300">+</button></div></td>
                            <td className="p-4 text-center">
                                <input type="checkbox" checked={item.active ?? true} onChange={(e) => updateStock(item.product_id, item.size, 'active', e.target.checked)} className="w-6 h-6 text-blue-600 rounded focus:ring-blue-500 cursor-pointer" />
                            </td>
                        </tr>
                        ))}
                    </tbody>
