...
export default function AdminPage() {
        ...
        if (supabase) {
            const channel = supabase
                ...
                .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
                    ...
                    fetchOrders(); // Forces the UI to refresh with the new order
                })
                .subscribe();
  ...
  useEffect(() => {
    ...
    if (orders.length > lastOrderCount.current && lastOrderCount.current !== 0) {
      ...
      const isRecent = (new Date() - new Date(newestOrder.created_at)) < 45000;
      
      if (isRecent && !newestOrder.printed) {
        console.log("New order detected - triggering auto-print.");
  ...
  const printLabel = async (order) => {
      ...
      try {
          ...
          if (!result.success) { alert("Print Error: " + (result.error || "Unknown")); return; }
          if (!isCloud) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 text-black font-sans">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === 'inventory' && (
            <div className="grid md:grid-cols-3 gap-6">
                <div className="md:col-span-1 space-y-6">
                    <div className="bg-white p-6 rounded-lg shadow border border-gray-200"><h2 className="font-bold text-xl mb-4">Add New Item</h2><form onSubmit={handleAddProductWithSizeUpdates} className="space-y-3"><div><label className="text-xs font-bold uppercase">ID</label><input className="w-full border p-2 rounded" placeholder="e.g. jogger_grey" value={newProdId} onChange={e => setNewProdId(e.target.value)} /></div><div><label className="text-xs font-bold uppercase">Display Name</label><input className="w-full border p-2 rounded" placeholder="e.g. Grey Joggers" value={newProdName} onChange={e => setNewProdName(e.target.value)} /></div><div><label className="text-xs font-bold uppercase">Type</label><select className="w-full border p-2 rounded bg-white" value={newProdType} onChange={e => setNewProdType(e.target.value)}><option value="top">Top</option><option value="bottom">Bottom</option></select></div><div><label className="text-xs font-bold uppercase">Price</label><input type="number" className="w-full border p-2 rounded" value={newProdPrice} onChange={e => setNewProdPrice(e.target.value)} /></div><button className="w-full bg-green-600 text-white font-bold py-2 rounded">Create Product</button></form></div>
                    <div className="bg-blue-50 p-6 rounded-lg shadow border border-blue-200"><h2 className="font-bold text-lg mb-2 text-blue-900">📦 Bulk Update</h2><button onClick={downloadTemplate} className="text-xs bg-white border border-blue-300 px-3 py-1 rounded text-blue-700 font-bold mb-4">⬇️ Download Stock</button><input type="file" onChange={handleBulkUpload} className="block w-full text-sm text-gray-500" /></div>
                </div>
                <div className="md:col-span-2 space-y-6"><div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300"><div className="bg-blue-900 text-white p-4 font-bold uppercase text-sm">Manage Prices</div><table className="w-full text-left text-sm"><thead className="bg-gray-100"><tr><th className="p-3">Product</th><th className="p-3">Price ($)</th><th className="p-3 text-right">Action</th></tr></thead><tbody>{products.map((prod) => (<tr key={prod.id} className="border-b"><td className="p-3 font-bold">{prod.name}</td><td className="p-3"><input type="number" className="w-20 border rounded p-1" value={prod.base_price} onChange={(e) => updatePrice(prod.id, e.target.value)} /></td><td className="p-3 text-right"><button onClick={() => deleteProduct(prod.id)} className="text-red-500 font-bold">🗑️</button></td></tr>))}</tbody></table></div><div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300"><div className="bg-gray-800 text-white p-4 font-bold uppercase text-sm">Manage Stock</div><table className="w-full text-left text-sm"><thead className="bg-gray-100"><tr><th className="p-4">Product</th><th className="p-4">Size</th><th className="p-4 text-center">Cost</th><th className="p-4">Stock</th></tr></thead><tbody>{inventory.map((item) => (<tr key={`${item.product_id}_${item.size}`} className="border-b"><td className="p-4 font-bold">{getProductName(item.product_id)}</td><td className="p-4">{item.size}</td><td className="p-4"><input type="number" className="w-16 border rounded text-center mx-auto block" value={item.cost_price || ''} onChange={(e) => updateStock(item.product_id, item.size, 'cost_price', parseFloat(e.target.value))} /></td><td className="p-4"><input type="number" className="w-16 border text-center font-bold" value={item.count} onChange={(e) => updateStock(item.product_id, item.size, 'count', parseInt(e.target.value))} /></td></tr>))}</tbody></table></div></div>
            </div> )}

        {activeTab === 'guests' && (<div className="max-w-4xl mx-auto"><div className="bg-white p-6 rounded-lg shadow mb-6 border border-gray-200"><h2 className="font-bold text-xl mb-4">Guests</h2><input type="file" onChange={handleGuestUpload} className="block w-full text-sm text-gray-500" /><button onClick={clearGuestList} className="mt-4 text-red-600 font-bold text-xs">🗑️ Clear All</button></div><div className="bg-white shadow rounded-lg overflow-hidden border border-gray-300"><table className="w-full text-left text-sm"><thead className="bg-gray-100"><tr><th className="p-4">Name</th><th className="p-4">Size</th><th className="p-4 text-center">Status</th><th className="p-4 text-right">Action</th></tr></thead><tbody>{guests.map((g) => (<tr key={g.id} className="border-b"><td>{g.name}</td><td>{g.size}</td><td className="text-center">{g.has_ordered ? 'REDEEMED' : 'Wait'}</td><td className="text-right"><button onClick={() => resetGuest(g.id)} className="text-blue-600 font-bold text-xs underline">Reset</button></td></tr>))}</tbody></table></div></div>)}
        ...
        {activeTab === 'settings' && (<div className="max-w-xl mx-auto bg-white p-8 rounded shadow border space-y-6"><h2 className="font-black text-2xl">Settings</h2><div><label className="font-bold text-xs uppercase text-gray-500">Event Name</label><input className="w-full border p-3 rounded" value={eventName} onChange={e => setEventName(e.target.value)} /></div><div className="bg-purple-50 p-6 rounded border border-purple-200"><p className="font-bold text-purple-900 mb-2">Cloud Print</p><div className="flex justify-between items-center mb-4"><span>Enable?</span><input type="checkbox" checked={pnEnabled} onChange={e => setPnEnabled(e.target.checked)} /></div><input className="w-full border p-2 mb-2 text-sm" placeholder="API Key" value={pnApiKey} onChange={e => setPnApiKey(e.target.value)}/><div className="flex gap-2"><input className="flex-1 border p-2 text-sm" placeholder="Printer ID" value={pnPrinterId} onChange={e => setPnPrinterId(e.target.value)} /><button onClick={discoverPrinters} className="bg-purple-600 text-white px-3 py-1 text-xs rounded font-bold">Discover</button></div></div><button onClick={saveSettings} className="w-full bg-blue-900 text-white py-4 font-black rounded shadow">Save Settings</button><button onClick={closeEvent} className="w-full bg-red-100 text-red-800 font-bold py-3 mt-10 rounded border border-red-200">🏁 Close Event</button></div>)}
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        {editingOrder.cart_data.map((item, idx) => (
                            <div key={idx} className="bg-white p-4 border rounded-lg shadow-sm space-y-4">
                                <div className="flex justify-between items-center pb-2 border-b"><span className="font-bold text-lg">{item.productName}</span><select className="border-2 p-1 rounded font-bold" value={item.size} onChange={(e) => handleEditItem(idx, 'size', e.target.value)}>{SIZE_ORDER.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                                <div><label className="text-xs font-bold uppercase text-gray-400">Design</label><select className="w-full border p-2 rounded" value={item.customizations?.mainDesign || ''} onChange={(e) => handleUpdateMainDesign(idx, e.target.value)}><option value="">None</option>{logos.filter(l => l.category === 'main').map(l => (<option key={l.id} value={l.label}>{l.label}</option>))}</select></div>
