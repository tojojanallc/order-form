...
export default function OrderForm() {
  ...
  // --- THE ONLY FIX APPLIED: CHANGED "sweat" TO "sweatpant" ---
  // This explicitly prevents sweatshirts/hoodies from showing the pants graphic.
  const isBottomSelected = selectedProduct ? (
    selectedProduct.type === 'bottom' || 
    (selectedProduct.name || '').toLowerCase().match(/jogger|pant|short|sweatpant/) || 
    (selectedProduct.id || '').toLowerCase().match(/jogger|pant|short|sweatpant/)
  ) : false;
  ...
  useEffect(() => {
    // 100% BULLETPROOF URL PARSER - NO LOCALSTORAGE CACHE
    let currentSlug = params?.slug;
    if (!currentSlug && typeof window !== 'undefined') {
        currentSlug = new URLSearchParams(window.location.search).get('event');
    }
  ...
  const getCheckoutSlug = () => {
      ...
      if (!slug && typeof window !== 'undefined') {
          slug = new URLSearchParams(window.location.search).get('event');
      }
      return slug || 'default';
  };
