...
export default function OrderForm() {
  ...
  // --- 100% BULLETPROOF DUPLICATE & SIZE MERGER ---
  // This groups ALL inventory by the Product NAME, completely bypassing duplicate IDs
  const activeSizesByName = {};
  const productObjByName = {};

  Object.keys(activeItems).forEach(key => {
      if (activeItems[key] !== true) return;
      
      const prod = products.find(p => key.startsWith(p.id + '_'));
      if (!prod) return;

      if (paymentMode === 'hosted' && (inventory[key] || 0) <= 0) return;

      const size = key.replace(prod.id + '_', '');
      
      if (!activeSizesByName[prod.name]) {
          activeSizesByName[prod.name] = new Set();
          productObjByName[prod.name] = prod;
      }
      activeSizesByName[prod.name].add(size);
  });

  // The visible products are now ONLY unique names that actually have active stock
  const visibleProducts = Object.values(productObjByName);
  ...
  // Pull sizes exclusively for the selected product name
  const visibleSizes = selectedProduct && activeSizesByName[selectedProduct.name] 
      ? Array.from(activeSizesByName[selectedProduct.name]).sort((a, b) => SIZE_ORDER.indexOf(a) - SIZE_ORDER.indexOf(b))
      : [];
  ...
  // STOCK CALCULATOR (Merges stock from all duplicate IDs so the math is perfect)
  const currentStock = (() => {
  ...
  const getCheckoutSlug = () => {
      ...
      return currentSlug || actualEventSlug || 'default';
  };
