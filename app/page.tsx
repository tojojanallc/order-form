...
const SIZE_ORDER = ['Youth XS', 'Youth S', 'Youth M', 'Youth L', 'Youth XL', 'YS', 'YM', 'YL', 'Adult XS', 'Adult S', 'S', 'Adult M', 'M', 'Adult L', 'L', 'Adult XL', 'XL', 'Adult XXL', '2XL', 'Adult 3XL', '3XL', 'Adult 4XL', '4XL'];
...
export default function OrderForm() {
  ...
  // Selection state — in order: product → color → size
  ...
  const isBottomSelected = (() => {
      ...
      if (n.includes('sweatshirt') || n.includes('hoodie') || id.includes('sweatshirt') || id.includes('hoodie')) return false; 
      if (t === 'bottom' || n.includes('jogger') || n.includes('pant') || n.includes('short') || id.includes('jogger') || id.includes('pant') || id.includes('short')) return true;
      return false;
  })();
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
        ...
        const querySlug = urlParams.get('event');
        if (querySlug) {
            currentSlug = querySlug;
        } else {
            const pathSegments = window.location.pathname.split('/').filter(Boolean);
            const lastSegment = pathSegments[pathSegments.length - 1];
            if (lastSegment && lastSegment !== 'kiosk') currentSlug = lastSegment;
        }
    }
    
    setActualEventSlug(currentSlug);
    ...
    const fetchData = async () => {
      ...
      const { data: logoData } = await supabase.from('logos').select('*').eq('active', true).eq('event_slug', currentSlug).order('sort_order');
      if (logoData) {
  ...
  // ============================================================
  // DROPDOWN LOGIC: Product → Color → Size
  // ============================================================

  // STEP 1: Unique products that have active inventory for this event
  const activeProductIds = [...new Set(eventInventory.map(item => item.product_id))];
  const visibleProducts = activeProductIds
    .map(id => globalProducts.find(p => p.id === id) || { id, name: id, base_price: 0 })
  ...
  // STEP 2: Unique colors available for the selected product
  // Only show color picker if more than one color exists
  ...
  // STEP 3: Sizes for selected product + color
  const visibleSizes = selectedProduct
    ? [...new Set(
        eventInventory
          .filter(inv => {
            if (inv.product_id !== selectedProduct.id) return false;
            // If this product has multiple colors, filter by selected color
            if (hasMultipleColors && selectedColor) return inv.color === selectedColor;
            return true;
          })
          .map(inv => inv.size)
      )].sort((a, b) => {
  ...
  // Auto-select first size when color or product changes
  useEffect(() => {
  ...
  // STEP 4: Find exact inventory row
  const currentInvRow = eventInventory.find(i => {
    if (i.product_id !== selectedProduct?.id) return false;
    if (i.size !== size) return false;
    if (hasMultipleColors && selectedColor) return i.color === selectedColor;
    return true;
  });
  ...
  const qtyInCart = cart.filter(item => {
    ...
    if (hasMultipleColors && selectedColor) return item.color === selectedColor;
    return true;
  }).length;
  ...
  const sendConfirmationSMS = async (name, phone) => {
      ...
      fetch('/api/send-sms', {
          ...
          body: JSON.stringify({ phone, message: `Hi ${name}! Thanks for your order from Lev Custom Merch at ${eventName}. We will text you again when it's ready for pickup!` })
  ...
  const handleAddToCart = () => {
    ...
    const newItem = {
      ...
      size,
      color: currentInvRow?.color || selectedColor || null,
  ...
  const handleTerminalCheckout = async () => {
    ...
    try {
        ...
        const payRes = await fetch('/api/terminal-pay', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ orderId, amount: calculateGrandTotal(), taxCollected: calculateTax(), deviceId: assignedTerminalId })
        });
  ...
  const decrementInventory = async (cartItems) => {
      for (const item of cartItems) {
          let query = supabase.from('inventory').select('count')
          ...
          if (current && current.count > 0) {
              let updateQuery = supabase.from('inventory').update({ count: current.count - 1 })
                .eq('event_slug', actualEventSlug)
                .eq('product_id', item.productId)
                .eq('size', item.size);
  ...
  if (visibleProducts.length === 0 && eventInventory.length > 0 && paymentMode !== 'hosted') {
    return <div className="p-10 text-center text-red-600 font-bold text-xl mt-10">No active products found for event: "{actualEventSlug}"</div>;
  }
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-300">
            ...
            <div className="p-6 text-white text-center relative" style={{ backgroundColor: headerColor }}>
                ...
                <p className="text-white text-opacity-80 text-xs font-mono mt-1">Event: {actualEventSlug}</p>
              ...
              {(paymentMode === 'retail' || selectedGuest) && (
                  <>
                    ...
                    <section className="bg-gray-50 p-4 rounded-xl border border-gray-300">
                        ...
                        {selectedGuest && (
                            <div className="bg-green-50 border border-green-200 p-4 rounded-lg mb-6 text-center shadow-sm">
                                ...
                                {selectedGuest.size 
                                  ? <p className="text-green-700 text-sm">We've pre-selected size <span className="font-bold bg-white px-2 py-0.5 rounded border border-green-300">{selectedGuest.size}</span> for you.</p>
                                  : <p className="text-green-700 text-sm">Please select your apparel below.</p>
                                }
                        ...
                        {!selectedProduct && eventInventory.length === 0 ? (
                            <div className="text-center py-8 text-gray-400 font-bold animate-pulse">Loading products...</div>
                        ) : !selectedProduct ? (
                            ...
                            <>
                                ...
                                {selectedProduct.image_url && (
                                  <div className="mb-4 bg-white p-2 rounded border border-gray-200 flex justify-center">
                                    <img src={selectedProduct.image_url} className="h-48 object-contain" />
                                  </div>
                                )}

                                {/* Stock badge */}
                                {isOutOfStock 
                                  ? <div className="bg-orange-100 text-orange-700 p-3 mb-4 rounded font-bold">⚠️ Out of Stock (Will Ship)</div>
                                  : <div className="bg-green-100 text-green-700 p-2 mb-4 text-xs font-bold rounded">✓ In Stock ({currentStock} available)</div>
                                }
                                
                                {/* ── DROPDOWNS: Product → Color → Size ── */}
                                <div className="space-y-4">

                                  {/* 1. Product */}
                                  <div>
                                    <label className="text-xs font-black text-gray-900 uppercase tracking-wide mb-1 block">Product</label>
                                    <select
                                      className="w-full p-4 border-2 rounded-xl bg-white text-black font-bold text-lg"
                                      value={selectedProduct?.id || ''}
                                      onChange={(e) => {
                                        const prod = visibleProducts.find(p => p.id === e.target.value);
                                  ...
                                  {/* 2. Color — only shown if product has multiple colors */}
                                  {hasMultipleColors && (
                                    <div>
                                      <label className="text-xs font-black text-gray-900 uppercase tracking-wide mb-1 block">Color</label>
                                      <select
                                        className="w-full p-4 border-2 rounded-xl bg-white text-black font-bold text-lg"
                                        value={selectedColor}
                                        onChange={(e) => {
                                  ...
                                  {(!hasMultipleColors || selectedColor) && (
                                    <div>
                                      <label className="text-xs font-black text-gray-900 uppercase tracking-wide mb-1 block">Size</label>
                                      <select
                                        className="w-full p-4 border-2 rounded-xl bg-white text-black font-bold text-lg"
                                        value={size}
                    ...
                    {selectedProduct && availableMainOptions.length > 0 && (
                        <section>
                            <div className="flex justify-between items-center mb-3 border-b border-gray-300 pb-2">
                              <h2 className="font-bold text-lg text-black">2. Choose Design</h2>
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-4">
                                <div className="col-span-2 grid grid-cols-2 gap-3">
                                    {availableMainOptions.map((opt) => (
                                        <button key={opt.label} onClick={() => setSelectedMainDesign(opt.label)} className={`border-2 rounded-xl p-2 flex flex-col items-center gap-2 transition-all active:scale-95 ${selectedMainDesign === opt.label ? 'border-green-600 bg-green-50 ring-2 ring-green-200' : 'border-gray-200 bg-white hover:border-gray-400'}`}>
                                            {opt.image_url 
                                              ? <img src={opt.image_url} className="h-20 w-full object-contain" />
                                ...
                                <div className="col-span-1">
                                  {(() => { 
                                    ...
                                    return <PlacementVisualizer garmentType={isBottomSelected ? 'bottom' : 'top'} logoSize={sizeFromDB} />; 
                                  })()}
                                </div>
                            </div>
                        </section>
                    ...
                    {selectedProduct && availableAccentOptions.length > 0 && (
                        <section>
                            <div className="flex justify-between items-center mb-3 border-b border-gray-300 pb-2">
                              <h2 className="font-bold text-lg text-black">Add Accents (Optional)</h2>
                              {showPrice && <span className="text-xs bg-blue-100 text-blue-900 px-2 py-1 rounded-full font-bold">+$5.00</span>}
                            </div>
                                    ...
                                    {logos.map((logo, index) => {
                                        ...
                                        return (
                                            <div key={index} className="flex items-center gap-3 bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                                                <div className="w-10 h-10 flex-shrink-0 border rounded bg-gray-50 flex items-center justify-center">
                                                  {currentImage ? <img src={currentImage} className="max-h-8 max-w-8" /> : <span className="text-xs">IMG</span>}
                                                </div>
                                                <div className="flex-1"><div className="text-sm font-bold">{logo.type}</div></div>
                                                <select className={`border-2 p-2 rounded-lg font-bold text-sm ${!logo.position ? 'border-red-400 bg-red-50 text-red-900' : 'border-gray-300 text-black'}`} value={logo.position} onChange={(e) => updateLogo(index, 'position', e.target.value)}>
                    ...
                    {selectedProduct && (showPersonalization || showNumbers) && (
                        <section>
                            <div className="flex justify-between items-center mb-3 border-b border-gray-300 pb-2">
                              <h2 className="font-bold text-lg text-black">3. Personalization</h2>
                              {showPrice && <span className="text-xs bg-blue-100 text-blue-900 px-2 py-1 rounded-full font-bold">+$5.00</span>}
                            </div>
                            ...
                            {names.map((nameItem, index) => (
                              <div key={`name-${index}`} className="flex flex-col md:flex-row gap-2 mt-4 bg-gray-100 p-3 rounded-xl">
                                  ...
                                  <select className="border-2 border-gray-300 p-3 rounded-lg md:w-48 bg-white text-black font-bold" value={nameItem.position} onChange={(e) => updateName(index, 'position', e.target.value)}>
                                    <option value="">Position...</option>
                            ...
                            {numbers.map((numItem, index) => (
                              <div key={`num-${index}`} className="flex flex-col md:flex-row gap-2 mt-4 bg-gray-100 p-3 rounded-xl">
                                  ...
                                  <select className="border-2 border-gray-300 p-3 rounded-lg md:w-48 bg-white text-black font-bold" value={numItem.position} onChange={(e) => updateNumber(index, 'position', e.target.value)}>
                                    <option value="">Position...</option>
                                        ...
                                        <div className="pt-2">
                                            ...
                                            {metallicHighlight && (
                                                <div className="animate-pulse-once">
                                                  ...
                                                  <input type="text" className="w-full p-3 border-2 border-blue-400 rounded-lg font-bold uppercase text-black" placeholder="ENTER NAME HERE" value={metallicName} onChange={(e) => setMetallicName(e.target.value)} />
                                                </div>
                                            )}
                                        </div>
                                    )}
            ...
            {(paymentMode === 'retail' || selectedGuest) && (
                <div className="p-6 sticky bottom-0 bg-white border-t flex justify-between items-center shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
                    <div className="text-3xl font-black text-blue-900">{showPrice ? `$${calculateItemTotal().toFixed(2)}` : 'Free'}</div>
                    <button onClick={handleAddToCart} className="bg-blue-900 text-white px-10 py-4 rounded-xl font-bold text-xl shadow-lg active:scale-95" disabled={!selectedProduct || !size || (hasMultipleColors && !selectedColor)}>
                      Add to Cart
                    </button>
                </div>
            )}
            ...
            <div className="bg-white shadow-xl rounded-2xl border border-gray-300 sticky top-4 overflow-hidden">
                ...
                {cart.length === 0 
                  ...
                  : cart.map((item) => (
                    <div key={item.id} className="border-b border-gray-200 pb-3 last:border-0 relative group">
                      ...
                      {item.color && <p className="text-sm text-gray-600 font-bold mt-1">Color: {item.color}</p>}
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-300">
                    ...
                    {cartRequiresShipping && paymentMode !== 'hosted' && (
                      <div className="bg-orange-50 border border-orange-200 p-3 rounded-xl mb-4 animate-pulse-once">
                    ...
                    {showPrice && (
                        <div className="mt-4 border-t-2 border-gray-200 pt-4 space-y-2 mb-6">
                            ...
                            {taxEnabled && taxRate > 0 && (
                                <div className="flex justify-between text-sm font-bold text-gray-600 uppercase">
                                    <span>Sales Tax ({taxRate}%)</span><span>${calculateTax().toFixed(2)}</span>
                                </div>
                            )}
...
const PlacementVisualizer = ({ garmentType, logoSize }) => {
  ...
  const renderTopZones = () => { 
      if (logoSize === 'large') return <rect x="75" y="70" width="50" height="50" fill={color} fillOpacity="0.5" rx="4"><animate attributeName="opacity" values="0.3;0.7;0.3" dur="2s" repeatCount="indefinite" /></rect>;
  ...
  const renderBottomZones = () => <circle cx="80" cy="50" r="10" fill={color} fillOpacity="0.5"><animate attributeName="opacity" values="0.3;0.7;0.3" dur="2s" repeatCount="indefinite" /></circle>;
